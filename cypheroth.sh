...
declare -a queries=(
    "All Objects that are members of High Value Groups;MATCH p=(n)-[:MemberOf*1..]->(m:Group {highvalue:true, domain:'$DOMAIN'}) SET n.highvalue=true RETURN labels(n) AS ObjectType,n.name AS ObjectName,m.name AS HighValueGroupName ORDER BY ObjectType ASC;AllHighValueObjects.csv"
    ...
    "Users that are admin on 1+ machines, sorted by admin count;MATCH (u:User {domain:'$DOMAIN'}) OPTIONAL MATCH (u)-[:AdminTo]->(c1:Computer) OPTIONAL MATCH (u)-[:MemberOf*1..]->(:Group)-[:AdminTo]->(c2:Computer) WITH u.name AS UserName, COLLECT(c1) + COLLECT(c2) AS tempVar UNWIND tempVar AS computers WITH UserName, COUNT(DISTINCT(computers)) as c WHERE c>0 RETURN UserName,c ORDER BY c DESC;UserAdminCount.csv"
    ...
    "Groups with Computers and Admins;MATCH (c:Computer)-[:MemberOf*1..]->(g:Group {domain:'$DOMAIN'}) WITH g MATCH (u:User)-[:MemberOf*1..]->(g) RETURN DISTINCT(g.name) as Name, g.domain AS Domain, g.highvalue AS HighValue, g.objectid AS SID, g.description AS Description, g.admincount AS AdminCount;GroupsWithCompsAndAdmins.csv"
    "Group Admin Info;MATCH (g:Group {domain:'$DOMAIN'}) WITH g OPTIONAL MATCH (g)-[:AdminTo]->(c1:Computer) WITH g,COUNT(c1) as explicitAdmins OPTIONAL MATCH (g)-[:MemberOf*1..]->(a:Group)-[:AdminTo]->(c2:Computer) WITH g,explicitAdmins,COUNT(DISTINCT(c2)) as unrolledAdmins RETURN g.name AS Name,explicitAdmins,unrolledAdmins, explicitAdmins + unrolledAdmins as totalAdmins ORDER BY totalAdmins DESC;GroupAdminInfo.csv"
    ...
    "Computers with Local Admin Data;MATCH (n)-[:AdminTo]->(c:Computer {domain:'$DOMAIN'}) WITH COUNT(DISTINCT(c)) as computersWithAdminsCount MATCH (c2:Computer {domain:'$DOMAIN'}) RETURN c2.name AS ComputerName, c.operatingsystem AS OperatingSystem;compsWithLocalAdminData.csv"
    "Computers with Session Data;MATCH (c:Computer {domain:'$DOMAIN'})-[:HasSession]->() WITH COUNT(DISTINCT(c)) as computersWithSessions MATCH (c2:Computer {domain:'$DOMAIN'}) RETURN c2.name AS ComputerName, c.operatingsystem AS OperatingSystem;compsWithSessionData.csv"
